...
TARBALL_URL=$(curl -sSL "$LATST_RELEASE_URL" | jq -r ".assets[] | select(.name | contains(\"_linux_${VERSION_CODENAME}\")) | .browser_download_url")
curl -SL "$TARBALL_URL" | tar xzf - -C "$INSTALL_DIR_PATH"
